...
<img src="/images/vibe-coding/claims-dashboard-top.png" alt="Claims Dashboard example — cards, charts, and filters" style={{width: '100%', borderRadius: '8px', border: '1px solid #e0e0e0', marginTop: '1rem'}} />
</a>
...
Start by telling Claude Code exactly what you want. Think like a product owner writing a brief.
...
<details>
<summary>What Claude Code will do</summary>
...
**Open the file in your browser** and take a look at what Claude Code built.
...
If you want to save your project and share it with colleagues, Claude Code can help you set up a GitHub repository.
...
### Step 3.5: Clear the conversation if Claude Code is going in circles

If Claude Code seems to be repeating the same fix or going in circles, type `/clear` to reset the conversation. This gives Claude Code a fresh perspective on the current state of your files without the baggage of a long debugging history. Your files on disk are not affected -- only the chat history is cleared. (Your CLAUDE.md rules are automatically reloaded.)
...
- Let Claude Code handle all technical implementation
