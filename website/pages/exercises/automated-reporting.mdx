...
1. Create a folder for this exercise (e.g. `automated-reporting/`) and inside it create a subfolder called `data/`
2. Download all five files into the `data/` folder:
   - [claims_q1_2025.csv](../data/exercise-4/claims_q1_2025.csv) | [claims_q2_2025.csv](../data/exercise-4/claims_q2_2025.csv) | [claims_q3_2025.csv](../data/exercise-4/claims_q3_2025.csv) | [claims_q4_2025.csv](../data/exercise-4/claims_q4_2025.csv) | <a href="../data/exercise-4/report_template.md" download>report_template.md</a>
3. Open your terminal in the `automated-reporting/` folder and start Claude Code
...
- Does Claude Code correctly identify all four quarters?
...
Claude Code generates a self-contained HTML file (`quarterly_report_charts.html`) with five Chart.js charts. The file includes:
...
- Did Claude Code reuse the same report structure, or did it invent a new one?
...
### "Claude Code says it cannot read the files"
...
This can happen if Claude Code misinterprets which column to use. Clarify in your prompt:
...
Check that your browser is not blocking the CDN script (e.g., Chart.js). If you are behind a corporate proxy, ask Claude Code to embed the library inline instead of loading from a CDN.
...
Check which Python version you have (`python --version` or `python3 --version`). The script may require `pandas` -- install it with `pip install pandas` if needed. If Claude Code generated the script using only the standard library, ensure the CSV parsing logic handles the date formats correctly.
...
**Next module:** [Module 5: Vibe Coding](/vibe-coding) — build full applications using natural language.
