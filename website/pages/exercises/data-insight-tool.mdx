...
1. Create a folder for this exercise (e.g. `data-insight/`) and inside it create a subfolder called `data/`
2. Download the four CSV files into the `data/` folder:
   - [market_penetration.csv](../data/exercise-2/market_penetration.csv) | [demographics.csv](../data/exercise-2/demographics.csv) | [accident_frequency.csv](../data/exercise-2/accident_frequency.csv) | [product_lines.csv](../data/exercise-2/product_lines.csv)
3. Open your terminal in the `data-insight/` folder and start Claude Code
...
- Does Claude Code identify that `region_code` is the join key across files?
...
- Does Claude Code handle the aggregation from product-line level to region level correctly?
...
- Does Claude Code explain its methodology, or does it just present numbers?

> **Tip:** Push Claude Code to refine the analysis: *"Weight the opportunity score more heavily toward regions where MIG already has some presence (market share > 0.1%) -- pure greenfield markets may be too risky."*
...
![Exercise 2 final browser result](./images/exercise-1-screenshot.jpg)
...
- Check if Claude Code is filtering to a single year. The `market_penetration.csv` file has data for both 2024 and 2025. Ask Claude: *"How many rows does the merged dataset have? I expected 68 regions."*

### "The opportunity score does not make sense"
- Ask Claude Code to explain its methodology: *"Walk me through how you calculated the opportunity score. What weight did you give each factor?"*
...
- Check if Claude Code used a CDN link for Chart.js. If you are offline, ask: *"Inline the Chart.js library directly in the HTML file instead of using a CDN."*
