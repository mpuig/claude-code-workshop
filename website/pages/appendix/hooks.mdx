...
| `~/.claude/settings.json` (Mac/Linux) or `%USERPROFILE%\.claude\settings.json` (Windows) | All your projects | No (local to your machine) |
...
- Configure hooks in `.claude/settings.json` (project, shared) or `~/.claude/settings.json` (`%USERPROFILE%\.claude\settings.json` on Windows) for personal, all-project hooks
