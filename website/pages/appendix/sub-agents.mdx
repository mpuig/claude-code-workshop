...
description: "How to use Claude Code sub-agents for parallel work, create custom agents for your team, and coordinate multi-agent teams for complex insurance workflows."
---
...
When Claude Code encounters a task that benefits from isolation -- exploring a large codebase, running a long test suite, researching a topic -- it can spawn a sub-agent using the **Task tool**. Each sub-agent:
...
Claude delegates to these automatically based on the task. You can also request them explicitly:
...
|----------|-------|---------|----------|
| `.claude/agents/` | Current project | Yes (commit to repo) | Highest |
| `~/.claude/agents/` (Mac/Linux) or `%USERPROFILE%\.claude\agents\` (Windows) | All your projects | No (local) | Lower |

Project agents (`.claude/agents/`) are ideal for team-shared roles. Commit them to version control so your team uses and improves them together.
...
| `description` | Yes | When Claude should delegate to this agent (natural language) |
...
Claude delegates automatically based on the `description` field. You can also be explicit:
...
Agent teams go beyond sub-agents. Instead of isolated workers reporting back to a parent, teams are **separate Claude Code instances** that communicate with each other through a shared task list and messaging system.
...
| **Teammates** | Separate Claude Code instances. Each claims tasks, works independently, messages others. |
...
Each worktree gets its own branch and working directory at `.claude/worktrees/<name>/`. Worktrees with no changes are cleaned up automatically; those with changes prompt you to keep or remove.
...
- Sub-agents are isolated Claude instances that keep verbose work out of your main conversation and return only relevant findings
- Built-in agents (Explore, Plan, General-purpose) handle common patterns automatically; custom agents in `.claude/agents/` add team-specific roles
- Custom agent definitions use YAML frontmatter + Markdown and can be committed to version control for team sharing
- Agent teams (experimental) enable multiple Claude instances to collaborate through shared task lists and direct messaging
...
- For a comparison between sub-agents in Claude Code and sub-agents in Claude Cowork, see [Appendix A: Claude Cowork](/appendix/claude-cowork)
