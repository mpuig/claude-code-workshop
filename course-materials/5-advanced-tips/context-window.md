...
The **context window** is Claude Code's short-term memory. It holds everything Claude Code can "see" at once: your messages, its responses, file contents, and system instructions. The current context window is approximately **200.000 tokens** (~400 pages of text).

Think of it like a desk:

- **Clean desk** = fresh session, Claude Code is fast and focused
- **Papers everywhere** = long session, Claude Code is slower and may lose track of earlier details
...
| Messages in session | What Claude processes per message | Effect |
|--------------------|--------------------------------|--------|
...
3. **Quality decreases:** With a crowded context, Claude may lose focus on earlier instructions or important details
...
- When Claude starts repeating itself or forgetting earlier instructions
...
- Before saving results that only exist in Claude's responses (copy them to a file first)
- When Claude is referencing earlier context that you still need

> **Tip:** Before clearing, ask Claude to save any important outputs to a file. That way you keep the results without keeping the full conversation history.

---

## How `/clear` Interacts with CLAUDE.md

When you use `/clear`:

- **Cleared:** All conversation history -- your messages and Claude's responses
- **Reloaded:** Your CLAUDE.md file is automatically reloaded, so project context is preserved

This is why CLAUDE.md is so valuable ([Module 1.2](/fundamentals/project-memory)). It ensures that essential project context survives a `/clear`. Put any instructions you want Claude Code to always follow into CLAUDE.md rather than repeating them in conversation.
...
4. **Clear** with `/clear`
5. **Start the next task** -- CLAUDE.md reloads automatically
...
- The context window (~200.000 tokens) is Claude Code's short-term memory -- it fills up during a session
...
- CLAUDE.md survives `/clear` -- use it for persistent instructions
