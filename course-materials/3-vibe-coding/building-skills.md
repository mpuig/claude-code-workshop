...
![Skills: Customize Claude to your Enterprise](images/skills-overview.png)
...
A **skill** is a set of instructions, packaged as a simple folder, that teaches Claude how to handle specific tasks or workflows. Skills allow you to customize Claude by teaching it your preferences, processes, and domain expertise once, rather than re-explaining them in every conversation.
...
> This section is based on Anthropic's official guide: [The Complete Guide to Building Skills for Claude](https://resources.anthropic.com/hubfs/The-Complete-Guide-to-Building-Skill-for-Claude.pdf). Refer to it for the full technical reference, including advanced patterns, testing strategies, and distribution.
...
1. **First Level (YAML Frontmatter):** Always loaded in Claude's system prompt. It provides just enough information for Claude to know *when* to use the skill.
2. **Second Level (`SKILL.md` body):** Loaded only when Claude deems the skill relevant based on the frontmatter. This contains the full instructions and guidance.
3. **Third Level (Linked files):** Additional files within the skill directory (`references/`, `scripts/`) that Claude can discover and navigate as needed.
...
- **Security:** Do not use XML angle brackets (`< >`) or the words "claude" or "anthropic" in the skill name.
...
- **Troubleshooting:** Common errors, why they happen, and how Claude should fix them.

---

## Insurance Examples

Here are three skills tailored to MIG's workflows. These illustrate the three common categories Anthropic has observed across skill builders.
...
Adds workflow guidance on top of a connected data source, teaching Claude *how* to analyse treaty data rather than just retrieve it.
...
3. **Include Error Handling:** If a skill relies on an MCP server, explicitly tell Claude what to do if the connection fails or an API returns an error.
4. **Prioritize Critical Info:** Put the most important instructions at the top under a `## Critical` header. Claude pays more attention to instructions at the beginning.

---

## The `skill-creator` Skill

You do not have to write skills from scratch. The `skill-creator` skill is built into Claude Code and available in Claude.ai. It can:
...
|----------|-------|----------------|
| Claude.ai Settings > Skills | Shared across Chat, Cowork, and Code Tab | Upload `.zip` or `.md` |
| `~/.claude/skills/` | All Claude Code projects on your machine | Place skill folder in directory |
| `.claude/skills/` (in project) | Only this specific project | Place skill folder in project |

For more on how skills work across Claude's different interfaces, see [Appendix A: Claude Cowork](../appendix-a-cowork/README.md).

---

## Further Reading

- [The Complete Guide to Building Skills for Claude](https://resources.anthropic.com/hubfs/The-Complete-Guide-to-Building-Skill-for-Claude.pdf) -- Anthropic's official reference covering planning, testing, distribution, and advanced patterns
- [anthropics/skills on GitHub](https://github.com/anthropics/skills) -- Anthropic's official skill repository with production-ready examples
