# Appendix A: Claude Cowork

> **Type:** Reference | **Prerequisites:** None

This course focuses on **Claude Code**, the terminal-based interface. But not everyone on your team works in a terminal. **Claude Cowork** is a visual, desktop-based agent that uses the same underlying model and the same skill format -- designed for people who prefer a graphical interface over the command line.
...
Cowork is not a chat interface with a new look. It is an **autonomous desktop agent** built into the Claude Desktop app.

When you open the Cowork tab, you give Claude access to a sandboxed Linux VM running on your machine. Inside that sandbox, Claude can write scripts, create files (Word documents, slide decks, spreadsheets, PDFs), and connect to services like Gmail, GitHub, and Slack through built-in connectors managed by Anthropic.
...
Both Chat and Cowork share conversation, file creation skills (DOCX, PPTX, XLSX, PDF), browser automation (Claude in Chrome), artifacts, connectors (Gmail, GitHub, Slack), and custom MCP servers. The full comparison across all four tools is in the reference table at the end of this page.
...
Both tools use the same Claude model. The difference is how you interact with them.

| | Claude Code | Claude Cowork |
|---|-------------|---------------|
| **Interface** | Terminal (command line) | Visual tab in Claude Desktop |
...
Cowork has a dedicated **Plugins** panel where you can browse, install, and create plugins from a visual interface. Each plugin bundles one or more **skills** -- reusable instruction sets that teach Claude how to approach specific tasks.
...
Skills load on demand. Claude reads a short description of each skill (~100 tokens) to decide which ones are relevant, then loads full instructions only when needed. This keeps your context window clean.

### Default Plugins

Cowork ships with 11 business-focused plugins from Anthropic covering productivity, product management, legal, finance, marketing, and data workflows. Claude Code's marketplace defaults to developer workflows (code review, frontend design, feature development).
...
> **Note:** Cowork and the Code Tab have separate plugin panels. Installing a plugin in one does not make it available in the other. Skills uploaded via Claude Desktop settings are shared across Chat, Cowork, and Code Tab.

### Where to Find More Skills

| Source | Description |
|--------|-------------|
| `github.com/anthropics/skills` | Anthropic's official skill repository |
| `github.com/anthropics/knowledge-work-plugins` | Cowork's default plugin registry (11 business-role plugins) |
| `github.com/anthropics/claude-code` | Developer-focused workflows (Code's marketplace) |
...
|------|-------------|----------------|
| **Remote connectors** | Cloud-hosted, OAuth-based. Toggle in Settings > Connectors. Gmail, GitHub, Slack, Notion, Figma, etc. | claude.ai + Claude Desktop |
| **Extensions** | Local MCP servers as `.mcpb` or `.dxt` bundles. Install via Settings > Extensions. Filesystem, Desktop Commander, PDF Tools, etc. | Claude Desktop only |
| **Custom connectors** | Your own remote MCP server via Settings > Connectors > Add custom. HTTP Streamable + OAuth. | claude.ai + Claude Desktop |
| **Custom MCP servers** | Edit `claude_desktop_config.json` directly. Any transport (stdio or HTTP Streamable). Full flexibility. | Claude Desktop only |

All four types appear in a unified "Connectors" interface with on/off toggles inside Claude Desktop.
...
> **Example for MIG:** You could allow Claude to search your Outlook inbox for policy renewal reminders but block it from sending replies automatically. Or connect a Google Drive MCP to pull claims data from shared team folders.

### MCP Config Across Tools

Adding an MCP server to Claude Desktop makes it available in Chat, Cowork, and Code Tab. However, it does **not** automatically apply to the Claude Code CLI. The CLI uses its own configuration.
...
**Desktop Commander** is an extension that gives Claude broad access to your local machine -- file management, system commands, and even installing other MCP servers.
...
> **Tip:** If you use the Claude in Chrome extension, disable it when not needed. Claude sometimes defaults to web-based actions even when a dedicated MCP connector is available.
...
Replace `{your_folder}` with a path on your machine (e.g., `Documents/claude-memory`).
...
> **Note:** Claude Code already has built-in memory management through its CLAUDE.md file. This section applies specifically to Cowork, which requires manual setup.

---

## Claude Chat vs. Cowork vs. Code: Reference Table
...
| **Browser automation** | Claude in Chrome | Claude in Chrome | Claude in Chrome | Claude in Chrome |
...
| **Sub-agents (parallel tasks)** | No | Parallel sub-agents | Task tool (up to 10 parallel) | Task tool + custom (`claude/agents/`) |
| | | | | |
| **Cross-session memory** | Built-in | No -- add via instructions | No -- add via instructions | No -- add via instructions |
| **Instructions** | Global + Project instructions | Global + folder instructions | Global + folder + CLAUDE.md | CLAUDE.md (project) + `~/.claude/CLAUDE.md` (global) |
...
| **Custom skills via filesystem** | No | No | `~/.claude/skills/` (global) | `~/.claude/skills/` (global) + `.claude/skills/` (project) |
| **Plugin UI panel** | No | Cowork-specific (default: knowledge-work-plugins) | Code Tab-specific (default: claude-code) | No |
| **Slash commands** | Reference by name only | Autocomplete | Autocomplete | Works but no autocomplete |
| | | | | |
| **Remote connectors (Gmail, Slack...)** | Yes | Yes | Yes | No -- use `claude mcp add` |
| **Extensions (.mcpb / .dxt)** | Desktop only | Desktop only | Desktop only | No |
| **Custom MCP servers (JSON config)** | Desktop only | Desktop only | Desktop config + `~/.claude.json` | `~/.claude.json` or `.mcp.json` (project) |
| **Project-scoped MCPs** | Global only | Global only | Global only | Yes -- per-project via `.mcp.json` |

> **Note:** Config files differ across tools. Chat/Cowork use `claude_desktop_config.json`. Code Tab shares that file plus `~/.claude.json`. Code CLI uses `~/.claude.json` (global) or `.mcp.json` (project). Adding an MCP to one does not make it available in the others.
...
- It uses the same Claude model and skill format as Claude Code, so knowledge transfers between tools
- Plugins and MCP connectors let you extend Cowork to work with email, cloud storage, and internal tools
- Per-tool permissions give you granular control over what Claude can and cannot do automatically
- Cross-session memory requires manual setup but is straightforward with Desktop Commander
- For insurance workflows involving documents, presentations, and spreadsheets, Cowork is often the better choice over Claude Code
- For development, data pipelines, and git-based workflows, Claude Code remains the right tool
